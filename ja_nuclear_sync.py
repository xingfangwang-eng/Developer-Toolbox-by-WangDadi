...
GIT_DIR = r"E:\Developer-Toolbox-by-WangDadi"
BATCH_SIZE = 30
...
def git_add_commit_push(processed_count):
    print(f"\n{'='*60}")
    print(f"[Sync] 已累计完成 {processed_count} 个文件，正在推送到 GitHub...")
    print(f"{'='*60}")

    commands = [
        ["git", "add", "."],
        ["git", "commit", "-m", f"Auto-sync: {processed_count} Japanese Souls Injected"],
    ...
    for attempt in range(3):
        try:
            for cmd in commands:
                ...
                if result.returncode != 0:
                    ...
                    if attempt < 2:
                        ...
                        with open("ja_sync_errors.log", "a", encoding="utf-8") as f:
                            f.write(f"SYNC FAILED at {processed_count}: {result.stderr}\n")
                        return False
            ...
            if attempt >= 2:
                with open("ja_sync_errors.log", "a", encoding="utf-8") as f:
                    f.write(f"SYNC TIMEOUT at {processed_count}\n")
                return False
        except Exception as e:
            print(f"Git error: {e}")
            if attempt >= 2:
                with open("ja_sync_errors.log", "a", encoding="utf-8") as f:
                    f.write(f"SYNC ERROR at {processed_count}: {e}\n")
                return False
        time.sleep(1)
...
def main():
    print("=" * 60)
    print("JA Nuclear Sync - 日语重写 + GitHub 自动同步")
    print("=" * 60)
    ...
    print(f"每 {BATCH_SIZE} 个文件自动 Git 同步一次")
    print("-" * 60)
    ...
    with tqdm(total=len(to_process), desc="重写进度", unit="file") as pbar:
        ...
        for i, filepath in enumerate(to_process):
            ...
            if batch_counter > 0 and batch_counter % BATCH_SIZE == 0:
                pbar.set_postfix({"状态": "正在同步 GitHub..."})
